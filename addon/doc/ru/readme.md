...
*   **Пакетное распознавание текста:** Добавлена поддержка выбора и обработки нескольких изображений или PDF-файлов одновременно. Файлы теперь обрабатываются в алфавитном порядке.
*   **Исправление совместимости:** Библиотека `markdown` включена в состав дополнения для устранения ошибок запуска (ModuleNotFoundError) в NVDA 2024.3+ и обеспечения стабильности во всех версиях.
*   **Безопасность кода:** Реализован более безопасный механизм импорта для предотвращения конфликтов с другими дополнениями или внутренними библиотеками NVDA.
*   **Перевод:** Исправлена проблема, из-за которой префикс "Custom:" в меню улучшения текста не переводился.
